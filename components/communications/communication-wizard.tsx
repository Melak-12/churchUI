...
export function CommunicationWizard({ step, data, memberStats, onUpdate, onPreview }: CommunicationWizardProps) {
  const [customMembers, setCustomMembers] = useState<any[]>([]);
  const [selectedMembers, setSelectedMembers] = useState<string[]>(data.customAudience || []);
  ...
  useEffect(() => {
    if (step === 1 && data.audience === 'CUSTOM' && customMembers.length === 0) {
  ...
  const insertVariable = (variable: string) => {
    ...
    if (textarea) {
      ...
      const newValue = data.body.slice(0, start) + variable + data.body.slice(end);
    ...
    switch (step) {
      ...
      case 1: // Audience Selection
        return (
          <div className="space-y-4">
            ...
            <RadioGroup
              value={data.audience}
              onValueChange={(value) => onUpdate({ audience: value as AudienceType })}
              className="space-y-2"
            >
      ...
      case 2: // Message Composition
        return (
          <div className="space-y-4">
            <div>
              <Label htmlFor="messageBody" className="text-base font-medium">Your Message</Label>
              <Textarea
                id="messageBody"
                value={data.body}
            ...
            {data.body && (
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-sm whitespace-pre-wrap text-gray-600">{messagePreview}</div>
              </div>
            )}
          </div>
        );
      ...
      case 3: // Schedule & Send
        return (
          <div className="space-y-4">
            <Label className="text-base font-medium">When should this be sent?</Label>
            <RadioGroup
            ...
            >
              ...
              <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
                ...
                <Label htmlFor="schedule" className="flex-1 cursor-pointer">
                  ...
                  <div className="text-sm text-gray-500">Choose date and time</div>
                </Label>
              </div>
            </RadioGroup>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {scheduledDate ? format(scheduledDate, "MMM dd") : "Pick date"}
                        </Button>
                      </PopoverTrigger>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {audienceOptions.find(opt => opt.value === data.audience)?.label}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Recipients:</span>
                    <span className="font-medium">{getMemberCount(data.audience)} members</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Timing:</span>
                    <span className="font-medium">
                      {data.scheduledAt 
                        ? format(new Date(data.scheduledAt), "MMM dd 'at' HH:mm")
                        : 'Send immediately'
                      }
                    </span>
                  </div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  <div className="text-sm whitespace-pre-wrap text-gray-700">{messagePreview || data.body}</div>
                </div>
              </div>
            </div>
