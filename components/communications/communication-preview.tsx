...
export function CommunicationPreview({ 
  ...
  const fetchCustomRecipients = async () => {
    ...
    try {
      const memberPromises = data.customAudience.map(id => apiClient.getMember(id));
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="recipients">Recipients ({memberCount})</TabsTrigger>
          </TabsList>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{getAudienceDescription(data.audience)}</p>
                  </div>
                  <div>
                            ...
                            <TableRow key={member.id}>
                              ...
                              <TableCell>
                                <div className="flex items-center space-x-1">
                                  {getEligibilityIcon(member.eligibility)}
                                  <span className="text-sm">{member.eligibility}</span>
                                </div>
                              </TableCell>
                            </TableRow>
                          ))}
