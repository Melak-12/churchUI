...
export function CommunicationEditForm({ initialData, onSave, onCancel, isLoading = false }: CommunicationEditFormProps) {
  ...
  const handleSave = () => {
    ...
    const saveData: Partial<Communication> = {
      ...
      body: data.body,
      customAudience: data.audience === 'CUSTOM' ? data.customAudience : undefined,
  ...
  const getAudienceCount = () => {
    switch (data.audience) {
      case 'ALL': return memberStats.total;
      case 'ELIGIBLE': return memberStats.eligible;
      case 'DELINQUENT_30': return memberStats.delinquent30;
      case 'DELINQUENT_60': return memberStats.delinquent60;
      case 'DELINQUENT_90': return memberStats.delinquent90;
      case 'CUSTOM': return data.customAudience.length;
      default: return 0;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <RadioGroup
            value={data.audience}
            onValueChange={(value) => updateData({ audience: value, customAudience: [] })}
            className="space-y-3"
          >
            {audienceOptions.map((option) => {
              const Icon = option.icon;
              const count = option.value === 'CUSTOM' ? data.customAudience.length : 
                           option.value === 'ALL' ? memberStats.total :
                           option.value === 'ELIGIBLE' ? memberStats.eligible :
                           option.value === 'DELINQUENT_30' ? memberStats.delinquent30 :
                           option.value === 'DELINQUENT_60' ? memberStats.delinquent60 :
                           option.value === 'DELINQUENT_90' ? memberStats.delinquent90 : 0;
            ...
            <div className="space-y-4 border-t pt-4">
              <div className="space-y-2">
                ...
                {errors.customAudience && (
                  <p className="text-sm text-red-600">{errors.customAudience}</p>
                )}
              </div>
              ...
              <Popover open={showCalendar} onOpenChange={setShowCalendar}>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {data.scheduledAt ? format(new Date(data.scheduledAt), "PPP 'at' p") : "Pick a date"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={data.scheduledAt ? new Date(data.scheduledAt) : undefined}
                    onSelect={(date) => {
                      if (date) {
                        const currentTime = data.scheduledAt ? new Date(data.scheduledAt) : new Date();
