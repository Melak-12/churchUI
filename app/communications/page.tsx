...
import { Plus, MessageSquare, AlertCircle, Filter, CheckCircle } from 'lucide-react';
...
export default function CommunicationsPage() {
  ...
  useEffect(() => {
    console.log('Filter useEffect running - communications count:', communications.length, 'statusFilter:', statusFilter);
    console.log('Communication IDs:', communications.map(c => c.id));
    console.log('Communication names:', communications.map(c => c.name));
    
    if (statusFilter === 'ALL') {
      setFilteredCommunications(communications);
      console.log('Set filteredCommunications to all communications:', communications.length);
    } else {
      const filtered = communications.filter(comm => comm.status === statusFilter);
      setFilteredCommunications(filtered);
      console.log('Set filteredCommunications to filtered:', filtered.length);
      console.log('Filtered communication IDs:', filtered.map(c => c.id));
      console.log('Filtered communication names:', filtered.map(c => c.name));
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    const communicationToDeleteObj = communications.find(c => c.id === communicationToDelete);
    ...
    console.log('Current filtered communications count:', filteredCommunications.length);
    ...
    try {
      ...
      console.log('Fresh communication IDs:', freshCommunications.map(c => c.id));
      ...
      console.log('Communications refreshed and refresh key set to:', Date.now());
          ...
          <div className="space-y-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-gray-600 mt-1">Manage SMS campaigns and member outreach</p>
              </div>
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        {/* Cohesive Header */}
        <div className="space-y-6">
          {/* Title and New Campaign Button - Integrated */}
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Communications</h1>
              <p className="text-gray-600 mt-1">Manage SMS campaigns and member outreach</p>
            ...
            <div className="flex items-center gap-2">
              <TwilioTest />
              <Button asChild variant="outline" className="h-9 text-gray-700 hover:text-gray-900 hover:bg-gray-50">
                <Link href="/communications/new" className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  <span className="hidden sm:inline">New SMS Campaign</span>
        ...
        {filteredCommunications.length > 0 ? (
          <div key={`grid-${refreshKey}`} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {statusFilter === 'ALL' ? 'No Communications Yet' : `No ${statusFilter.toLowerCase()} communications`}
              </h3>
              <p className="text-gray-600 text-center mb-6">
                {statusFilter === 'ALL' 
                  ? 'Create your first SMS campaign to start reaching out to your community members.'
                  : `No communications found with status "${statusFilter.toLowerCase()}". Try changing the filter or create a new campaign.`
                }
              </p>
              <Button asChild variant="outline" className="h-10 text-gray-700 hover:text-gray-900 hover:bg-gray-50">
                <Link href="/communications/new" className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  {statusFilter === 'ALL' ? 'Create First Campaign' : 'Create New Campaign'}
                </Link>
              </Button>
            </CardContent>
          </Card>
